...
//				String sql = "select distinct CHANNEL_REF_KEY,DOCUMENT_ID,MAJOR_VERSION from DOCUMENT_DETAILS where DOCUMENT_STATUS='LIVE' ";
	...
	public List<DocumentDetails> getDocumentsListForAddingReviewDate()
	{
		Connection conn = null;
		ResultSet rs = null;
		PreparedStatement pstmt=null;
		...
		{
			conn = getDestinationConnection();
			String sql = "select CHANNEL_REF_KEY,DOCUMENT_ID,LOCALE,MAJOR_VERSION,DISPLAY_REVIEW_DATE from DOCUMENT_DETAILS "
					+ "where DOCUMENT_STATUS='LIVE' ";
			logger.info("getDocumentsListForAddingReviewDate :: SQL :: >"+ sql);
			pstmt = conn.prepareStatement(sql);
			rs=pstmt.executeQuery();
			DocumentDetails details = null;
			while(rs.next())
			{
				...
				details.setMajorVersion(rs.getString("MAJOR_VERSION"));
				details.setChannelRefKey(rs.getString("CHANNEL_REF_KEY"));
				details.setDisplayReviewDate(rs.getTimestamp("DISPLAY_REVIEW_DATE"));
				if(null==documentsList || documentsList.size()<=0)
				{
					documentsList = new ArrayList<DocumentDetails>();
				}
			...
			if(null!=documentsList && documentsList.size()>0)
			{
				logger.info("getDocumentsListForAddingReviewDate :: Total Documents Found are :: >"+ documentsList.size());
			}
			else
			{
				logger.info("getDocumentsListForAddingReviewDate :: No Documents Found.");
			}
		}
		catch(Exception e)
		{
			Utilities.printStackTraceToLogs(ReadTransactionDAO.class.getName(), "getDocumentsListForAddingReviewDate()", e);
		}
		finally
		{
			try
